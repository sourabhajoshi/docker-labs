...
ARG is used to define build-time variables in a Dockerfile. It works only during ```docker build```. It is NOT available after the container starts (unless passed to ENV).

The ARG instruction defines build-time variables that are available only during the docker build process. It is commonly used for versioning and dynamic base images. Unlike ENV, ARG variables are not available in the running container unless explicitly passed to ENV.

ARG instruction build ಸಮಯದಲ್ಲಿ ಮಾತ್ರ available ಇರುವ variable define ಮಾಡುತ್ತದೆ. Version control ಮತ್ತು dynamic base image ಗಾಗಿ ಬಳಸಲಾಗುತ್ತದೆ. ENV ಗಿಂತ ಭಿನ್ನವಾಗಿ, container run ಆದ ಮೇಲೆ ARG available ಇರುವುದಿಲ್ಲ.
```
#Basic syntax
ARG VARIABLE_NAME
...
when we run ```docker build --build-arg APP_VERSION=2.0 -t myapp .``` Output during build is ```Building version 2.0```.

ARG must be declared before using it.
```
#wrong : ERROR
